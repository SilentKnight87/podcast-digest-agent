...
from agents.base_agent import (  # Assuming base events for now
    BaseAgent,
    BaseAgentEvent,
    BaseAgentEventType,
)
...
from google.genai.types import Content, PartDict
from google.genai.types import GenerateContentResponse as GenerationResponse
...
class TestSummarizerAgent:
    ...
    async def test_summarizer_agent_run_async_basic(self, summarizer_agent):
        ...
        assert (
            input_transcript in call_args[0].parts[0].text
        ), "Input transcript not found in LLM call prompt"
        assert (
            summarizer_agent.instruction in call_args[0].parts[0].text
        ), "Agent instruction not found in LLM call prompt"
        ...
        assert (
            progress_event.type == BaseAgentEventType.PROGRESS
        ), "First event type should be PROGRESS"
        assert "status" in progress_event.payload, "PROGRESS event payload missing 'status'"
        assert (
            progress_event.payload["status"] == "Starting summarization"
    ...
    async def test_summarizer_agent_run_async(self, summarizer_agent):
        ...
        input_transcript = (
            "This is a long transcript... it contains many details... please summarize."
        )
        ...
        mock_candidate.content.parts = [mock_part]
        mock_llm_response.candidates = [mock_candidate]
        summarizer_agent.llm.generate_content_async.return_value = (
            mock_llm_response  # Mock non-streaming call
        )
