...
async def test_agent_run_with_valid_script(tmp_path):
    ...
    print(
        f"[DEBUG] test_agent_run_with_valid_script: agent.tools = {[t.name for t in agent.tools]}"
    )
    ...
    with patch("src.tools.audio_tools.GenerateAudioSegmentTool.run", new=mock_generate), patch(
        "src.tools.audio_tools.CombineAudioSegmentsTool.run", new=mock_combine
    ):
        ...
        with patch(
            ...
            mock_llm_response_obj.text = (
                "LLM response (not directly used for orchestration by the new run method)"
            )
            ...
            assert (
                generate_calls_kwargs_list[0].get("output_filepath") == expected_segment_filepath_0
            )
            ...
            assert (
                generate_calls_kwargs_list[1].get("output_filepath") == expected_segment_filepath_1
            )

            mock_combine.assert_called_once_with(
                segment_filepaths=segment_paths, output_dir=str(test_output_dir)
            )
...
async def test_agent_run_with_empty_script(tmp_path):
    ...
    with patch(
        "google.generativeai.GenerativeModel.generate_content_async"
    ) as mock_generate_content:  # Kept for now
...
async def test_agent_handles_segment_generation_error(tmp_path):
    ...
    agent = AudioGenerator()
    print(
        f"[DEBUG] test_agent_handles_segment_generation_error: agent.tools = {[t.name for t in agent.tools]}"
    )
    ...
    with patch("src.tools.audio_tools.GenerateAudioSegmentTool.run", new=mock_generate):
        ...
        with patch(
            ...
            mock_llm_response.text = (
                "LLM fallback response for segment generation error (may not be used)"
            )
            mock_generate_content.return_value = mock_llm_response
...
async def test_agent_handles_combination_error(tmp_path):
    ...
    agent = AudioGenerator()
    print(
        f"[DEBUG] test_agent_handles_combination_error: agent.tools = {[t.name for t in agent.tools]}"
    )
    ...
    segment_paths = [
        str(
            test_output_dir / "segments" / "segment_1.mp3"
        ),  # Ensure these are plausible if tools were real
    ...
    with patch("src.tools.audio_tools.GenerateAudioSegmentTool.run", new=mock_generate), patch(
        ...
        with patch(
            ...
            mock_llm_response = AsyncMock()
            mock_llm_response.text = (
                "LLM fallback (may not be used by new run method for this error)"
            )
            mock_generate_content.return_value = mock_llm_response
            ...
            expected_segment_filepath_A0 = str(
                test_output_dir / "segments" / "temp_segment_A_0.mp3"
            )
            assert (
                generate_calls_kwargs_list[0].get("output_filepath") == expected_segment_filepath_A0
            )
            expected_segment_filepath_B1 = str(
                test_output_dir / "segments" / "temp_segment_B_1.mp3"
            )
            assert (
                generate_calls_kwargs_list[1].get("output_filepath") == expected_segment_filepath_B1
            )
...
async def test_agent_handles_llm_error(tmp_path):
    ...
    with patch(
        ...
        mock_generate_content.side_effect = Exception(
            "LLM API Error"
        ...
        agent.tools = [
            tool for tool in original_tools if tool.name != "generate_audio_segment"
        ]  # Ensure this uses the name as defined in the tool class
        ...
        assert (
            "generate_audio_segment not found" in result["error"].lower()
        )  # Ensure this matches the (new) agent error message format
...
async def test_audio_tools_unit_tests():
    ...
    with patch(
        "google.cloud.texttospeech_v1.TextToSpeechAsyncClient", new_callable=AsyncMock
    ) as mock_client:
        ...
        with patch(
            "aiofiles.open", return_value=mock_async_file_context
        ) as mock_aiofiles_open_func:
    ...
    with patch("os.path.exists", return_value=True), patch("os.makedirs"), patch(
    ...
    ) as mock_from_mp3, patch("src.tools.audio_tools.datetime") as mock_datetime:
        # Configure mocks
        ...
        with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread:
            ...
            output_path = await combine_audio_segments_tool.run(
                segment_filepaths=["/tmp/segment_1.mp3", "/tmp/segment_2.mp3"], output_dir="/output"
            )
