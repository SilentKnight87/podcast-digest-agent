...
async def test_agent_run_with_valid_video_id():
    ...
    def mock_run(*args, **kwargs):
        return {"success": True, "transcript": mock_transcript, "error": None}
...
async def test_agent_run_with_invalid_video_id():
    ...
    # Create a mock function that will replace the run method
    def mock_run(*args, **kwargs):
        return {"success": False, "transcript": None, "error": "No transcript found"}
    ...
    with patch("src.tools.transcript_tools.FetchTranscriptTool.run", side_effect=mock_run):
        ...
        with patch("google.generativeai.GenerativeModel.generate_content_async") as mock_generate:
            mock_response = AsyncMock()
            mock_response.text = (
                "I'm sorry, I couldn't find a transcript for that video. Error: No transcript found"
            )
            mock_generate.return_value = mock_response
...
async def test_agent_run_with_multiple_videos():
    ...
    mock_transcripts = {
        "video1": {"success": True, "transcript": "[00:01] Video 1 transcript", "error": None},
        "video2": {"success": True, "transcript": "[00:01] Video 2 transcript", "error": None},
    }
...
async def test_transcript_tool_with_real_exception():
    ...
    with patch("youtube_transcript_api.YouTubeTranscriptApi.get_transcript") as mock_get_transcript:
        mock_get_transcript.side_effect = TranscriptsDisabled(
