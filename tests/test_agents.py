...
# Agents to test
from src.agents.base_agent import BaseAgent, DEFAULT_MODEL_ID, Tool
...
async def test_summarizer_agent_run_async():
    ...
    mock_llm_response_content = genai.Content(role='model', parts=[genai.Part(text=expected_summary)])
...
async def test_synthesizer_agent_run_async():
    ...
    # Mock the event that run_async should yield
    mock_llm_response_content = genai.Content(role='model', parts=[genai.Part(text=expected_dialogue_json)])
    # Create the event *without* is_final_response
    mock_final_event = Event(content=mock_llm_response_content, author=agent.name)
