...
async def _broadcast_task_update(task_id: str):
    ...
    if task_status:
        try:
            ...
            await ws_manager.broadcast_to_task(task_id, task_status.model_dump())
...
def schedule_broadcast(task_id: str):
    ...
    try:
        ...
        logger.warning(
            f"No running asyncio loop to schedule broadcast for task {task_id}. Update won't be pushed via WebSocket immediately from this context."
        )


def create_initial_task_status(
    task_id: str, video_url: HttpUrl, request_data: ProcessUrlRequest
) -> TaskStatusResponse:
    ...
    initial_agents = [
        AgentNode(
            id="youtube-node",
            name="YouTube Video",
            description="Initial YouTube video source.",
            type="input_source",
            status="pending",
            icon="Youtube",
            progress=0,
        ...
        AgentNode(
            id="summarizer-agent",
            name="Summarizer",
            description="Generates a summary from the transcript.",
            type="summarization",
            status="pending",
            icon="Brain",
            progress=0,
        ...
        AgentNode(
            ...
            name="Dialogue Synthesizer",
            description="Converts summary to dialogue script.",
            type="synthesis",
            status="pending",
            icon="MessageSquare",
            progress=0,
        ...
        AgentNode(
            ...
            description="Generates audio from the dialogue script.",
            type="tts",
            status="pending",
            icon="Mic",
            progress=0,
        ...
        AgentNode(
            id="ui-player",
            name="UI/Player",
            description="Final output for the user.",
            type="output_display",
            status="pending",
            icon="PlayCircle",
            progress=0,
    ...
    initial_data_flows = [
        DataFlow(
            id=str(uuid.uuid4()),
            from_agent_id="youtube-node",
            to_agent_id="transcript-fetcher",
            data_type="video_url",
            status="pending",
        ),
        DataFlow(
            id=str(uuid.uuid4()),
            from_agent_id="transcript-fetcher",
            to_agent_id="summarizer-agent",
            data_type="transcript",
            status="pending",
        ),
        DataFlow(
            id=str(uuid.uuid4()),
            from_agent_id="summarizer-agent",
            to_agent_id="synthesizer-agent",
            data_type="summary",
            status="pending",
        ),
        DataFlow(
            id=str(uuid.uuid4()),
            from_agent_id="synthesizer-agent",
            to_agent_id="audio-generator",
            data_type="dialogue_script",
            status="pending",
        ),
        DataFlow(
            id=str(uuid.uuid4()),
            from_agent_id="audio-generator",
            to_agent_id="ui-player",
    ...
    task_status = TaskStatusResponse(
        ...
        processing_status=ProcessingStatus(
            status="queued", start_time=now_utc_iso, overall_progress=0
        ),
...
def add_new_task(video_url: HttpUrl, request_data: ProcessUrlRequest) -> dict[str, Any]:
    ...
    process_response = {
        ...
        "video_details": video_details.model_dump(),  # Convert Pydantic model to dict for response
    }
...
def update_task_processing_status(
    task_id: str,
    new_status: str,
    ...
    if task:
        for agent in task.agents:
            if agent.id == agent_id:
                ...
                add_timeline_event(
                    task_id,
                    f"AGENT_{new_status.upper()}",
                    f"Agent {agent.name} status: {new_status}",
                    agent_id=agent_id,
                )
                break
        schedule_broadcast(task_id)  # Broadcast after update
        ...
        for agent in task.agents:
            if agent.id == agent_id:
                if agent.logs is None:
                    agent.logs = []
                agent.logs.append(
                    AgentLog(timestamp=datetime.now(UTC).isoformat(), level=level, message=message)
                )
                break
        schedule_broadcast(task_id)  # Broadcast after update
        ...
        for flow in task.data_flows:
            if flow.from_agent_id == from_agent_id and flow.to_agent_id == to_agent_id:
                ...
                add_timeline_event(
                    task_id,
                    "DATA_FLOW_UPDATE",
                    f"Data flow {from_agent_id} -> {to_agent_id} status: {new_status}",
                )
                break
        schedule_broadcast(task_id)  # Broadcast after update


def add_timeline_event(task_id: str, event_type: str, message: str, agent_id: str | None = None):
    task = _tasks_store.get(task_id)
    if task:
...
def set_task_completed(task_id: str, summary: str, audio_url: str):
    ...
    if task:
        ...
        task.processing_status.estimated_end_time = datetime.now(UTC).isoformat()  # Rough estimate
...
def set_task_failed(task_id: str, error_message: str):
    ...
    if task:
        ...
        task.processing_status.overall_progress = (
            task.processing_status.overall_progress
        )  # Keep current progress or set to specific value
