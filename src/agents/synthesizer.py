...
class SynthesizerAgent(BaseAgent):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            name="SynthesizerAgent", instruction=DEFAULT_SYNTHESIZER_INSTRUCTION, **kwargs
        )
    ...
    async def run_async(self, summaries_json: str) -> AsyncGenerator[BaseAgentEvent, None]:
        ...
        yield BaseAgentEvent(
            type=BaseAgentEventType.PROGRESS, payload={"status": "Starting dialogue synthesis"}
        )
        ...
        try:
            ...
            yield BaseAgentEvent(
                type=BaseAgentEventType.ERROR, payload={"error": f"Invalid input JSON: {e}"}
            )
            return
        ...
        if not summaries:
            ...
            yield BaseAgentEvent(
                type=BaseAgentEventType.RESULT, payload={"dialogue": []}
        ...
        try:
            ...
            try:
                ...
                if dialogue_script and not all(
                    isinstance(item, dict) and "speaker" in item and "line" in item
                    for item in dialogue_script
                ):
                    raise ValueError(
                        "Parsed JSON list items do not match expected format {'speaker': ..., 'line': ...}."
                    )

                logger.info(
                    f"{self.name}: Dialogue script generated and parsed successfully ({len(dialogue_script)} lines)."
                )
                yield BaseAgentEvent(
                    type=BaseAgentEventType.RESULT, payload={"dialogue": dialogue_script}
                )

            except json.JSONDecodeError as e:
                logger.error(
                    f"{self.name}: Failed to parse LLM response as JSON: {e}. Raw text: {raw_dialogue_text[:500]}"
                )
                error_message = f"Error: Failed to parse dialogue script from LLM response - {e}"
                yield BaseAgentEvent(
                    type=BaseAgentEventType.ERROR,
                    payload={"error": error_message, "raw_response": raw_dialogue_text},
                )
            except ValueError as e:
                logger.error(
                    f"{self.name}: Parsed JSON structure validation failed: {e}. Raw text: {raw_dialogue_text[:500]}"
                )
