        ...
        if task_id in self.active_connections:
            if websocket in self.active_connections[task_id]:
                ...
                if not self.active_connections[task_id]:  # Clean up task_id if no more connections
                    ...
                    logger.info(
                        f"No more connections for task_id: {task_id}, removing from manager."
                    )
            else:
                logger.warning(
                    f"WebSocket {websocket.client} not found in task_id {task_id} during disconnect."
                )
        else:
            logger.warning(
                f"Task_id {task_id} not found in active_connections during disconnect for {websocket.client}."
            )
        ...
        if task_id in self.active_connections:
            ...
            for connection in self.active_connections[task_id]:
                try:
                    ...
                    logger.error(
                        f"Error sending message to WebSocket {connection.client} for task {task_id}: {e}"
                    )
