...
export class WebSocketManager {
  ...
  private async checkBackendHealth(): Promise<boolean> {
    try {
      ...
      console.log(
        `[WebSocket] Backend health check: ${isHealthy ? "HEALTHY" : "UNHEALTHY"}`,
      );
      return isHealthy;
    } catch (error) {
  ...
  private async establishConnection(): Promise<void> {
    if (
      ...
      console.log(
        `[WebSocket] Cannot establish connection - invalid task ID: ${this.taskId}`,
      );
      return;
    }
    ...
    if (!isBackendHealthy) {
      console.log(
        "[WebSocket] Backend not healthy, delaying connection attempt...",
      );
    ...
    try {
      ...
      this.socket.onmessage = (event) => {
        try {
          ...
          console.log(
            `[WebSocket] Raw message that failed parsing: ${event.data}`,
          );
        }
      };
      ...
      this.socket.onerror = (event) => {
        console.error(
          "[WebSocket] Connection error occurred. This is often due to network issues or the server being unavailable.",
        );
        console.log(
          `[WebSocket] Error details - Ready State: ${this.socket?.readyState}, URL: ${wsUrl}`,
        );
        console.log(
          "[WebSocket] This may be due to Docker container not being ready. Will retry...",
        );
  ...
  private attemptReconnect(): void {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.error(
        "[WebSocket] Maximum reconnection attempts reached. Giving up.",
      );
    }
  }
  ...
  sendPing(): void {
    if (this.socket && this.socket.readyState === WebSocket.OPEN) {
      ...
      console.log(
        `[WebSocket] Cannot send ping. Socket state: ${this.socket.readyState}`,
      );
      ...
      if (
        ...
        console.log(
          "[WebSocket] Socket closed or closing during ping. Attempting reconnect...",
        );
