    ...
    return () => {
      ...
      if (audioRef.current) {
        ...
        if (audioRef.current.dataset.blobUrl) {
          console.log(
            "[PlayDigestButton] Revoking blob URL:",
    ...
    return (
      <div className="text-center p-8">
        <p className="text-muted-foreground">
          Audio digest is ready, but URL is missing.
        </p>
        <p className="text-sm text-muted-foreground mt-2">
          URL value: {JSON.stringify(audioUrl)}
        </p>
      </div>
    );
  }
      ...
      try {
        ...
        testAudio.addEventListener("canplaythrough", () => {
          ...
          testAudio
            ...
            .catch((err) => {
              console.error(
                "[PlayDigestButton] Test audio playback failed:",
                err,
              );
              URL.revokeObjectURL(blobUrl);
            });
    ...
    if (audioUrl) {
      try {
        ...
        if (!actualResponse.ok) {
          console.error(
            `[PlayDigestButton] Actual audio fetch failed: ${actualResponse.status} ${actualResponse.statusText}`,
          );
          return;
        }
        ...
        if (hex.startsWith("49 44 33")) {
          ...
          hex.startsWith("ff fb") ||
          hex.startsWith("ff fa") ||
          hex.startsWith("ff f3")
        ) {
          console.log(
            "[PlayDigestButton] File starts with MPEG frame header (MP3)",
          );
        ...
        actualAudio.addEventListener("loadstart", () =>
          console.log("[Actual] loadstart"),
        );
        actualAudio.addEventListener("canplaythrough", () => {
          ...
          actualAudio
            ...
            .then(() => {
              console.log(
                "[PlayDigestButton] Actual audio played successfully",
              );
              setTimeout(() => {
                actualAudio.pause();
                URL.revokeObjectURL(blobUrl);
            ...
            .catch((err) => {
              console.error(
                "[PlayDigestButton] Actual audio playback failed:",
                err,
              );
              URL.revokeObjectURL(blobUrl);
            });
      ...
      <audio
        ...
        ref={(el) => {
          ...
          if (el && (!audioRef.current || audioRef.current.src !== el.src)) {
            console.log(
              "[PlayDigestButton] Setting audio element ref with src:",
              el.src,
            );
            audioRef.current = el;
            ...
            el.addEventListener("loadedmetadata", () => {
              console.log(
                "[PlayDigestButton] Audio metadata loaded. Duration:",
                el.duration,
              );
            ...
            el.addEventListener("error", (e) => {
              console.error(
                "[PlayDigestButton] Audio element error:",
                el.error,
              );
              ...
              console.log(
                "[PlayDigestButton] Audio element error - attempting direct fetch of file",
              );
              ...
              fetch(retryUrl)
                ...
                .then((blob) => {
                  console.log(
                    "[PlayDigestButton] Creating blob URL from blob:",
                    {
                      size: blob.size,
                      type: blob.type,
                    },
                  ...
                  console.log(
                    "[PlayDigestButton] Set audio src to blob URL:",
                    blobUrl,
                  );
                })
                .catch((error) => {
          ...
          onClick={() => {
            ...
            if (isPlaying) {
              ...
              audioRef.current.play().catch((error) => {
                console.error("[PlayDigestButton] Error playing audio:", error);
                toast.error(
                  `Error playing audio: ${error.message || "Unknown error"}`,
                );
              });
              setIsPlaying(true);
            }
