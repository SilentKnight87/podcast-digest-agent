...
const mockAgents: AgentNode[] = [
  {
    ...
    logs: [
      {
        timestamp: "2023-07-01T15:30:02Z",
        level: "info",
  ...
  },
  {
    ...
    logs: [
      {
        timestamp: "2023-07-01T15:30:10Z",
        level: "info",
        message: "Starting transcript download.",
      },
      {
        timestamp: "2023-07-01T15:30:55Z",
        level: "info",
  ...
  },
  {
    ...
    logs: [
      {
        timestamp: "2023-07-01T15:31:05Z",
        level: "info",
      ...
      },
      {
        ...
        level: "info",
        message: "Analyzing text segments...",
      },
    ],
  ...
  },
  {
    ...
    progress: 0,
    description:
      "Converts the summary into a natural-sounding dialogue script.",
  },
  {
    id: "audio-generator",
    ...
    progress: 0,
    description:
      "Generates the final audio output from the dialogue script using Text-to-Speech.",
  },
  {
    id: "ui-player",
    name: "UI/Player",
    ...
    progress: 0,
    description:
      "Delivers the generated audio digest to the user interface for playback.",
  },
];
...
const mockDataFlows: DataFlow[] = [
  {
    id: "flow1",
    fromAgentId: "youtube-node",
    toAgentId: "transcript-fetcher",
    dataType: "video_url",
    status: "completed",
  },
  {
    id: "flow2",
    fromAgentId: "transcript-fetcher",
  ...
  },
  {
    ...
    toAgentId: "synthesizer-agent",
    dataType: "summary",
    status: "pending",
  },
  {
    id: "flow4",
    fromAgentId: "synthesizer-agent",
    toAgentId: "audio-generator",
    dataType: "dialogue",
    status: "pending",
  },
  {
    id: "flow5",
    fromAgentId: "audio-generator",
    toAgentId: "ui-player",
    dataType: "audio_file",
    status: "pending",
  },
...
const ProcessingVisualizer: React.FC<ProcessingVisualizerProps> = ({
  ...
  console.log("[ProcessingVisualizer] Received props:", {
    ...
    dataFlows: dataFlows?.map(
      (f) => `${f.fromAgentId}->${f.toAgentId}: ${f.status}`,
    ),
    usingMockData: agents === mockAgents,
  });

  const [selectedAgent, setSelectedAgent] = React.useState<AgentNode | null>(
    null,
  );

  const nodePositions = agents.reduce(
    (acc, agent, index) => {
  ...
  const getStatusColor = (status: AgentNode["status"]) => {
    switch (status) {
      ...
      case "running":
        return "border-blue-500 text-blue-500 animate-pulse";
      case "completed":
  ...
  const getFlowColor = (dataType: DataFlow["dataType"]) => {
    switch (dataType) {
      case "transcript":
        return "stroke-primary fill-primary shadow-primary";
      case "summary":
        return "stroke-secondary fill-secondary shadow-secondary";
      case "audio_file":
        return "stroke-accent fill-accent shadow-accent";
      default:
    ...
    <div className="w-full h-[450px] bg-card/70 backdrop-blur-md p-4 rounded-lg shadow-lg relative overflow-x-auto overflow-y-hidden flex flex-col border border-border/40">
      <div className="flex-grow relative">
        <svg
          width={svgWidth}
          height="100%"
          aria-labelledby="processing-visualizer-title"
        >
          <title id="processing-visualizer-title">
              ...
              <React.Fragment key={`flow-group-${flow.id}`}>
                ...
                {(flow.status === "transferring" ||
                  ...
                  <motion.circle
                    ...
                    animate={{
                      opacity:
                        flow.status === "transferring" ? [0.4, 0.8, 0.4] : 0.2,
                      scale:
                        flow.status === "transferring" ? [0.8, 1.2, 0.8] : 1,
                    }}
                    transition={{
                      duration: 2,
                      repeat: Number.POSITIVE_INFINITY,
                    ...
                    <motion.circle
                      ...
                      animate={{
                        ...
                        transition: {
                          ...
                          delay:
                            index *
                            (flow.status === "transferring" ? 0.3 : 0.5),
                          repeatType: "loop",
                        },
                      }}
                      style={{
              ...
              >
                ...
                >
                  <div className="flex flex-col items-center justify-center w-full h-full p-1">
                    ...
                    >
                      <IconComponent
                        className={`w-8 h-8 transition-all ${statusColorClasses.split(" ")[1]}`}
                      />
                    </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
              <p>
                ...
                <span
                  className={`capitalize font-medium ${selectedAgent.status === "running" ? "text-blue-500" : selectedAgent.status === "completed" ? "text-green-500" : selectedAgent.status === "error" ? "text-red-500" : "text-gray-500"}`}
                >
            ...
            {selectedAgent.metrics &&
              Object.keys(selectedAgent.metrics).length > 0 && (
                <div className="mb-3">
                  <h4 className="text-sm font-semibold text-foreground mb-1">
                    Metrics:
                  </h4>
                  <ul className="list-disc list-inside pl-1 text-xs text-muted-foreground space-y-0.5">
                    {Object.entries(selectedAgent.metrics).map(
                      ([key, value]) => (
                        <li key={key}>
                          <strong className="text-foreground/80">{key}:</strong>{" "}
                          {value}
                        </li>
                      ),
                    )}
              ...
              <div>
                ...
                <div className="max-h-28 overflow-y-auto bg-muted/50 p-2 rounded text-xs space-y-1 font-mono scrollbar-thin scrollbar-thumb-muted-foreground/50 scrollbar-track-transparent">
                  {selectedAgent.logs.slice(-10).map((log, index) => (
                    <p
                      key={`${log.timestamp}-${index}-${log.message.slice(0, 10)}`}
                      className={`${log.level === "error" ? "text-red-400" : log.level === "warn" ? "text-yellow-400" : "text-muted-foreground/90"}`}
                    >
                      <span className="text-foreground/60">
                        [{new Date(log.timestamp).toLocaleTimeString()}]
                      </span>{" "}
                      <span
                        className={`font-semibold ${log.level === "error" ? "text-red-500" : log.level === "warn" ? "text-yellow-500" : "text-sky-500"}`}
                      >
