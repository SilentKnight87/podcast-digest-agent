...
const mapApiResponseToWorkflowState = (
...
export const WorkflowProvider: FC<WorkflowProviderProps> = ({ children }) => {
  ...
  const initialAgents = useMemo(
    () =>
      initialMockData.activeTask
        ? initialMockData.activeTask.agents.map(getInitialAgentState)
        : [],
    [],
  );
  ...
  const initialDataFlows = useMemo(
    () =>
      initialMockData.activeTask
        ? initialMockData.activeTask.dataFlows.map(getInitialDataFlowState)
        : [],
    [],
  );
    ...
    if (isProcessing && workflowState?.processingStatus.startTime) {
      ...
      elapsedTimeIntervalRef.current = setInterval(() => {
        setWorkflowState((prevState) => {
          if (!prevState || !prevState.processingStatus.startTime)
            return prevState;
          ...
          return {
            ...
            processingStatus: {
              ...
              elapsedTime: calculateElapsedTime(
                prevState.processingStatus.startTime,
              ),
            },
          };
        });
  ...
  useEffect(() => {
    ...
    if (!isInitializedRef.current) {
      if (
        !workflowState ||
        (workflowState.processingStatus.status !== "completed" &&
          !workflowState.outputUrl)
      ) {
        console.log(
          "[WORKFLOW_CONTEXT] Not initialized and condition met, calling resetToInitialState",
        );
        resetToInitialState();
      }
    ...
    api
      ...
      .then((response) => {
        ...
        const pollInterval = setInterval(() => {
          ...
          api
            ...
            .then((statusResponse) => {
              ...
              const pollState = mapApiResponseToWorkflowState(
                statusResponse.data,
              );
              ...
              if (
                statusResponse.data.processing_status.status === "completed" ||
                statusResponse.data.processing_status.status === "failed"
              ) {
                console.log("[WORKFLOW] Task completed, stopping poll");
                console.log(
                  "[WORKFLOW] Final audio URL:",
                  statusResponse.data.audio_file_url,
                );
        ...
        const unsubscribe = websocketManager.subscribe((data) => {
          ...
          console.log(
            "[WORKFLOW] Raw audio_file_url from API:",
            data.audio_file_url,
          );
          ...
          try {
            ...
            console.log("[WORKFLOW] Mapped state:", {
              ...
              dataFlows: newState.dataFlows.map(
            ...
            if (
              ...
              console.log(
                "[WORKFLOW] Processing completed with status:",
                data.processing_status.status,
              );
              console.log(
                "[WORKFLOW] Audio file URL from API:",
                data.audio_file_url,
              );
              console.log(
                "[WORKFLOW] Mapped audio URL in state:",
                newState.outputUrl,
              );
              console.log(
                "[WORKFLOW] All agents status:",
                newState.agents.map((a) => `${a.id}: ${a.status}`),
              );
        ...
        api
          ...
          .then((statusResponse) => {
            ...
            const initialState = mapApiResponseToWorkflowState(
              statusResponse.data,
            );
          ...
          .catch((error) => {
            console.error(
              "[WORKFLOW] Error fetching initial task status:",
              error,
            );
